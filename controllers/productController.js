...
const createProduct = asyncWrapper(async (req, res) => {
    const { title, description, price, availableInStock, totalRating, ratingCount } = req.body;
    const product = await Product.create({ title, description, price, availableInStock, totalRating, ratingCount });
    console.log('Created product: ', product?.firstName);
...
const searchProducts = asyncWrapper(async (req, res) => {
    ...
    if (!keyword) {
        return res.status(400).json({ success: false, error: 'Keyword parameter is required for search' });
    }
